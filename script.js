...
function movePlayer(direction) {
  if (direction === "up" && PLAYER.getBoundingClientRect().y >= 0) {
    playerY -= 20;
  } else if (
    direction === "down" &&
    PLAYER.getBoundingClientRect().y + PLAYER.clientHeight < 720
  ) {
    playerY += 20;
  }
...
function moveOpponent() {
  if (currentBallX > document.body.clientWidth / 2) {
    if (
      BALL.getBoundingClientRect().y + BALL.clientHeight >=
      OPPONENT.getBoundingClientRect().y + OPPONENT.clientHeight
    ) {
      opponentY += 3;
    } else if (
      BALL.getBoundingClientRect().y < OPPONENT.getBoundingClientRect().y
    ) {
      opponentY -= 3;
    }
  } else {
...
function ballCollisionPlayers() {
  if (
    BALL.getBoundingClientRect().right >=
      OPPONENT.getBoundingClientRect().left &&
    BALL.getBoundingClientRect().y + BALL.clientHeight >=
      OPPONENT.getBoundingClientRect().y &&
  ...
  if (
    ...
    BALL.getBoundingClientRect().y <=
      PLAYER.getBoundingClientRect().y + PLAYER.clientHeight
  ) {
    directionX = -directionX;
  }
...
function score() {
  ...
  if (currentBallX > document.body.clientWidth) {
    scorePlayerValue += 1;
    console.log(scorePlayerValue);
  }
